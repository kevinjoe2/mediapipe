...
class SubRectExtractorGlSlr {
 public:
  ...
  static absl::StatusOr<SubRectExtractorGlSlr> Create(
      const mediapipe::GlContext& gl_context, bool input_starts_at_bottom,
      BorderMode border_mode);

 private:
  explicit SubRectExtractorGlSlr(tflite::gpu::gl::GlProgram program,
...
absl::Status SubRectExtractorGlSlr::ExtractSubRectToBuffer(
    const tflite::gpu::gl::GlTexture& texture,
...
absl::StatusOr<SubRectExtractorGlSlr> SubRectExtractorGlSlr::Create(
    const mediapipe::GlContext& gl_context, bool input_starts_at_bottom,
    BorderMode border_mode) {
  ...
  return SubRectExtractorGlSlr(std::move(program), workgroup_size,
                            use_custom_zero_border, border_mode);
}

class GlProcessorSlr : public ImageToTensorConverter {
 public:
  absl::Status Init(CalculatorContext* cc, bool input_starts_at_bottom,
    ...
    return gl_helper_.RunInGlContext([this, input_starts_at_bottom,
      ...
      ASSIGN_OR_RETURN(
          auto extractor,
          SubRectExtractorGlSlr::Create(gl_helper_.GetGlContext(),
                                     input_starts_at_bottom, border_mode));
      extractor_ = absl::make_unique<SubRectExtractorGlSlr>(std::move(extractor));
      return absl::OkStatus();
    });
  }
  ...
  absl::StatusOr<Tensor> Convert(const mediapipe::Image& input,
    ...
    LOG(ERROR) << "SEGUNDO: (" << tensor.shape().dims[0] << "," << tensor.shape().dims[1] << "," << tensor.shape().dims[2] << "," << tensor.shape().dims[3] << ")";

    return tensor;
  }

  ~GlProcessorSlr() override {
 ...
 private:
  ...
  std::unique_ptr<SubRectExtractorGlSlr> extractor_;
...
CreateImageToGlBufferTensorConverterSlr(CalculatorContext* cc,
                                     bool input_starts_at_bottom,
                                     BorderMode border_mode) {
  auto result = absl::make_unique<GlProcessorSlr>();
